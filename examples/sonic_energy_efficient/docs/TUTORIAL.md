...
A small PyTorch toolbox that lets you bolt a **trainable residual MLP** on top of a **frozen ONNX policy**, train it with PPO (or any on-policy RL), and then **fuse the base + residual back into a single ONNX** for drop-in deployment.
...
Output is a single ONNX with the same input/output signature as `model_decoder.onnx`. Replace the decoder file in your deploy stack and nothing else changes.
...
1. Replace `model_decoder.onnx` in your deploy stack with `fused_best.onnx`.
2. Optional: re-export to TensorRT (`.engine`) with `trtexec` for lower inference latency. Pin TRT ≥ 10.13 — earlier versions have a broken op that affects this graph.
...
|---|---:|---:|---:|
| Mean power | 476 W | 357 W | **−24.9%** |
| Forward velocity | 2.30 m/s | 1.90 m/s | closer to cmd (|overshoot|: 0.60 → 0.20) |
...
| | Baseline (SONIC) | + Residual | Δ |
|---|---:|---:|---:|
| Mean power | 825 W | 699 W | **−15.2%** |
| Forward velocity | 2.28 m/s | 2.58 m/s | **faster** (closer to cmd) |
| Rolling σ(vx) | 0.080 | 0.055 | −31% |
| mean \|vx − cmd\| | 0.72 | 0.42 | **−42%** |
...
- Training curves generated by `tools/plot_training_curves.py` — IsaacLab EVAL at 25-iter intervals.
- Best iter ~1000 (E=950 W in IsaacLab, corresponds to 699 W in MuJoCo — the ~26% sim2sim gap is mostly that IsaacLab's weighted energy counts hip_pitch/ankles at 2× while MuJoCo eval is unweighted).
- Divergence begins ~iter 1500; final iteration is worse than best. **Use best checkpoint.**

### Per-joint breakdown (cmd=3.0)

Biggest reductions: `L_hip_roll` (−58%), `R_hip_roll` (−38%), `L_shoulder_pitch` (−17%), `L_hip_pitch` (−11%). The residual learned to reduce lateral sway and redirect energy into more efficient forward propulsion. Generate your own table with `tools/mujoco_joint_power_plot.py`.

---

## 12. Pitfalls & fixes — the full catalog

These are the traps we hit building this integration. Each one cost at least a day:
